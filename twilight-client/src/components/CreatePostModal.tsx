      ...
      return (
        <Modal onClose={() => props.setIsOpen(false)}>
          <form className="flex flex-col gap-2" onSubmit={(e) => post(e)}>
            {commonHeader}
            <textarea aria-required={true} aria-label="Post Content" required onChange={(e) => setTextContent(e.target.value)} placeholder="Write something..." className="min-h-[300px]" />
