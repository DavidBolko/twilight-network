  ...
  return (
    <nav className="hidden xl:flex flex-col sticky top-0 py-2 w-fit max-w-[200px] h-screen overflow-y-auto gap-2">
      ...
      <Link
        ...
        className="text-2xl px-2 md:text-4xl font-bold"
      ...
      {user && (
        <div className="flex flex-col overflow-y-auto gap-2">
          <CommunityList
            title="Owned"
            allowCreate={true}
            data={communities.data?.filter((c) => c.members.some((m) => m.id === user.id && m.isCreator)) ?? []}
          />
        ...
        {user ? (
          <>
            <Link to="/user/$id" params={{ id: user.id }} search={{ tab: "Posts" }} className="btn btn-muted w-full justify-start">
              <img src={user.avatar ? getFromCdn(user.avatar) : "/anonymous.png"} className="avatar size-8" alt="" />
              <span className="truncate">{user.userName}</span>
            </Link>
            <Link to="/auth/logout" className="btn btn-danger w-full justify-start">
              <LogOut size={18} /> Logout
            </Link>
          </>
        ) : (
          <Link to="/auth/login" search={{ redirect: "/" }} className="btn btn-primary w-full">
