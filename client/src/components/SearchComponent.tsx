    ...
    queries: [
      {
        ...
        enabled,
        queryFn: () => api.get<Community[]>(`c?query=${encodeURIComponent(q)}`),
      },
      {
        queryKey: ["search-users", q],
        enabled,
        queryFn: () => api.get<User[]>(`users/search?query=${encodeURIComponent(q)}`),
      },
    ],
  });
          ...
          <ul className="flex flex-col gap-4">
            {communities.length > 0 && (
              <>
                ...
                {communities.map((c) => (
                  <li key={`com-${c.id}`}>
                    <Link to="/communities/$id"  search={{ posts: "hot", time: "all" }} params={{ id: String(c.id) }} className="flex gap-2 items-center hover:opacity-90" onClick={() => setOpen(false)}>
            ...
            {users.length > 0 && (
              <>
                {communities.length > 0 && <hr className="divider-top my-1" />}
                ...
                {users.map((u) => (
                  <li key={`user-${u.id}`}>
                    <Link to="/user/$id" params={{ id: String(u.id) }} className="flex gap-2 items-center hover:opacity-90" onClick={() => setOpen(false)}>
                      <img src={u.avatar ? getFromCdn(u.avatar) : "/anonymous.png"} className="w-8 h-8 rounded-full object-cover" alt="" />
